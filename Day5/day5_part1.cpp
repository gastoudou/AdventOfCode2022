...
namespace day_5_part_1
{
	...
	void process( const std::vector< std::string >& inputs, data& data )
	{
		...
		for ( size_t i = header_line_id + 2; i < inputs.size(); ++i )
		{
			...
			{
				data.crates[ to_to_move ].push( data.crates[from_to_move].top() );
...
void day_5_run_part_1()
{
	day_5_part_1::data data;

	day_5_part_1::process( day_5_part_1::load_inputs(), data );
	day_5_part_1::display_result( data );
}