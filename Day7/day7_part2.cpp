...
namespace day_7_part_2
{
	...
	struct data
	{
		...
		void get_all_sizes( const directory& current, int memory_need )
		{
			for ( auto dir : current.get_directories() )
			{
				get_all_sizes( dir, memory_need );
		...
		void output_all_dir_size( int memory_need )
		{
			get_all_sizes( root, memory_need );
			...
			std::cout << "Mini max size: " << max_size << std::endl;
		}
	};
	...
	void display_result( data& data )
	{
		...
		std::cout << "Current used space: " << data.root.get_size() << std::endl;
		
		int free_space = 70000000 - data.root.get_size();
		std::cout << "Current unused space: " << free_space << std::endl;

		int to_free_space = 30000000 - free_space;
		std::cout << "Space to free: " << to_free_space << std::endl;

		data.output_all_dir_size( to_free_space );
	}
}

void day_7_run_part_2()
{
	day_7_part_2::data data;

	day_7_part_2::process( day_7_part_2::load_inputs(), data );
	day_7_part_2::display_result( data );
}